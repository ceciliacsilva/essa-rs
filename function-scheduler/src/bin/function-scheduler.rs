...
use essa_common::{
    ...
    executor_run_r_topic, scheduler_function_call_subscribe_topic, scheduler_function_call_topic,
    scheduler_run_module_topic, scheduler_run_r_function_call_topic, scheduler_run_r_topic,
};
...
async fn run() -> anyhow::Result<()> {
    ...
    // subscribe to remote function call requests issued by WASM functions
    let r_function_calls = zenoh
        .declare_queryable(scheduler_run_r_topic(zenoh_prefix))
        .res()
        .await
        .map_err(|e| anyhow::anyhow!(e))
        .context("failed to subscribe to function calls")?;

    let mut r_function_calls = r_function_calls.clone().into_stream().fuse();
    ...
    loop {
        select! {
            ...
            r_query = r_function_calls.select_next_some() => {
                call_r_function(r_query, zenoh.clone(), zenoh_prefix)
                    .await
                    .context("failed to run r function")?
            }
            complete => break,
        }
    }
...
async fn call_r_function(
    ...
    // forward the request to the selected executor
    let task = async move {
        let topic = executor_run_r_topic(&zenoh_prefix, r_executor_id, &function, &args);
        ...
        // TODO: this may be wrong. This is here to "fix" the problem:
        // Same problem as describe in the function above.
        // XXX: this could be query.key_expr()??
        let key = scheduler_run_r_function_call_topic(&zenoh_prefix, &function, &args);
        match reply.sample {
            Err(e) => eprintln!("`{key}` reply.sample is Err. Error = {e}"),
            Ok(value) => {
                let sample = Sample::try_from(key, value).expect("Cannot create a `Sample`");
